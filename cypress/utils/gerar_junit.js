  ...
  if (scenarioJson.properties) {
    for (var propertyName in scenarioJson.properties) {
      if (scenarioJson.properties.hasOwnProperty(propertyName)) {
        scenarioOutput[1].properties.push(createProperty(propertyName, scenarioJson.properties[propertyName]));
      }
    }
  }
...
function cucumberJunit(cucumberRaw, options) {
  ...
  if (cucumberRaw && cucumberRaw.toString().trim() !== "") {
    cucumberJson = JSON.parse(JSON.stringify(cucumberRaw));
  ...
  let xmlContent = xml({ testsuites: output }, options);

  const fs = require("fs");
  const xmlFilePath = "cucumber_report.xml";
  fs.writeFile(xmlFilePath, xmlContent, (err) => {
    if (err) {
      console.error("Ocorreu um erro ao escrever no arquivo XML:", err);
    } else {
      console.log("Arquivo XML criado e conteúdo escrito com sucesso.");
    }
  });
